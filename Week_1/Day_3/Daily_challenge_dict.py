...
def clean_price(money_string):
    return int("".join([c for c in money_string if c.isdigit()]))

def get_affordable_items(items_purchase, str_wallet):
    basket = [] 
    wallet = clean_price(str_wallet)
    
    for product, price_str in items_purchase.items():
...
input_items = {"Water": "$1", "Bread": "$3", "TV": "$1,000", "Fertilizer": "$20"}
wallet = "$300"
print(get_affordable_items(input_items, wallet))
